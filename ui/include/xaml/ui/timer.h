...
#define XAML_TIMER_VTBL(type)                                       \
    ...
    XAML_PROP(interval, type, XAML_CSTD size_t*, XAML_CSTD size_t); \
...
EXTERN_C XAML_UI_API xaml_result xaml_timer_new_interval(XAML_CSTD size_t, xaml_timer**) XAML_NOEXCEPT;

#ifdef __cplusplus
inline xaml_result xaml_timer_new_interval(std::chrono::milliseconds ms, xaml_timer** ptr) noexcept
{
    return xaml_timer_new_interval((std::size_t)ms.count(), ptr);
}
#endif // __cplusplus
