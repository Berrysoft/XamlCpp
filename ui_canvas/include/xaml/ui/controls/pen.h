...
XAML_CLASS(xaml_solid_pen, { 0xd0044425, 0xe477, 0x412a, { 0x9d, 0x48, 0xaf, 0xa5, 0x0b, 0xfa, 0x62, 0xa0 } })

#define XAML_SOLID_PEN_VTBL(type)           \
    XAML_VTBL_INHERIT(XAML_PEN_VTBL(type)); \
    XAML_PROP(color, type, xaml_color*, xaml_color)

XAML_DECL_INTERFACE_(xaml_solid_pen, xaml_pen)
{
    XAML_DECL_VTBL(xaml_solid_pen, XAML_SOLID_PEN_VTBL);
};

EXTERN_C XAML_UI_CANVAS_API xaml_result XAML_CALL xaml_solid_pen_new(xaml_color, double, xaml_solid_pen**) XAML_NOEXCEPT;
