...
struct xaml_type_guid<xaml_key_value_pair<TKey, TValue>>
{
    ...
    #define __XAML_KEY_VALUE_PAIR_T_T_TYPE(tkey, tvalue) typedef xaml_key_value_pair<tkey, tvalue> xaml_key_value_pair__##tkey##____##tvalue##__;
#else
    #define XAML_KEY_VALUE_PAIR_T_T_NAME(tkey, tvalue) xaml_key_value_pair__##tkey##____##tvalue##__

    #define __XAML_KEY_VALUE_PAIR_T_T_TYPE(tkey_name, tkey_interface, tvalue_name, tvalue_interface) \
...
#endif // __cplusplus
#define XAML_KEY_VALUE_PAIR_T_T_TYPE(tkey, tvalue) __XAML_KEY_VALUE_PAIR_T_T_TYPE(tkey, tvalue)
...
{
    XAML_MAP_VIEW_T_T_VTBL(xaml_map_view, TKey, xaml_interface_t<TKey>, TValue, xaml_interface_t<TValue>);
};
...
#endif // __cplusplus
#define XAML_MAP_VIEW_T_T_TYPE(tkey, tvalue) __XAML_MAP_VIEW_T_T_TYPE(tkey, tvalue)

__XAML_TYPE_NAME_BASE(xaml_hasher, { 0xa7f9b6eb, 0xa71a, 0x4d5a, { 0x84, 0x54, 0x28, 0x83, 0x94, 0x1f, 0xb2, 0xb0 } })

#define XAML_HASHER_T_VTBL(type, TN, TI)            \
    XAML_VTBL_INHERIT(XAML_OBJECT_VTBL(type));      \
    XAML_METHOD(hash, type, TI, XAML_STD int32_t*); \
    XAML_METHOD(equals, type, TI, TI, bool*)

#ifdef __cplusplus
XAML_DECL_INTERFACE_T_(xaml_hasher, xaml_object, XAML_HASHER_T_VTBL)

    #define XAML_HASHER_T_NAME(type) xaml_hasher<type>

    #define __XAML_HASHER_T_TYPE(type) typedef xaml_hasher<type> xaml_hasher__##type##__;
#else
    #define XAML_HASHER_T_NAME(type) xaml_hasher__##type##__

    #define __XAML_HASHER_T_TYPE(type_name, type_interface) \
        XAML_DECL_INTERFACE_T_(xaml_hasher, type_name, XAML_HASHER_T_VTBL, type_name, type_interface)
#endif // __cplusplus
...
__XAML_TYPE_NAME_BASE(xaml_map, { 0xe457ba53, 0x03d1, 0x4047, { 0xac, 0x85, 0x83, 0xdd, 0x85, 0x34, 0x6c, 0x7e } })

#define XAML_MAP_T_T_VTBL(type, TKeyN, TKeyI, TValueN, TValueI)                      \
    XAML_VTBL_INHERIT(XAML_MAP_VIEW_T_T_VTBL(type, TKeyN, TKeyI, TValueN, TValueI)); \
    XAML_METHOD(insert, type, TKeyI, TValueI, bool*);                                \
    XAML_METHOD(remove, type, TKeyI);                                                \
    XAML_METHOD(clear, type);                                                        \
    XAML_METHOD(get_hasher, type, XAML_HASHER_T_NAME(TKeyN)**)

#ifdef __cplusplus
template <typename TKey, typename TValue>
struct xaml_map : xaml_map_view<TKey, TValue>
{
    XAML_MAP_T_T_VTBL(xaml_map, TKey, xaml_interface_t<TKey>, TValue, xaml_interface_t<TValue>);
};
...
{
    static constexpr xaml_guid value = xaml_guid_xaml_map;
};

    #define XAML_MAP_T_T_NAME(tkey, tvalue) xaml_map<tkey, tvalue>

    #define __XAML_MAP_T_T_TYPE(tkey, tvalue) typedef xaml_map<tkey, tvalue> xaml_map__##tkey##____##tvalue##__;
#else
    #define XAML_MAP_T_T_NAME(tkey, tvalue) xaml_map__##tkey##____##tvalue##__
