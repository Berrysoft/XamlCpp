...
{
    optional<string_view> option::find_short_arg(uchar_t name) const noexcept
    {
    ...
    optional<string_view> option::find_long_arg(ustring_view_t name) const noexcept
    {
        auto it = m_long_args.find((ustring_t)name);
    ...
    shared_ptr<option> option::add_arg(uchar_t short_name, ustring_view_t long_name, string_view prop_name, ustring_view_t help_text)
    {
        if (short_name || !long_name.empty())
        {
            auto key = hash<string_view>{}(prop_name);
            m_properties.emplace(key, prop_name);
            m_entries.emplace(prop_name, option_entry{ short_name, (ustring_t)long_name, (ustring_t)help_text });
    ...
    basic_ostream<uchar_t>& option::print_help(basic_ostream<uchar_t>& stream) const
    {
        ...
        for (auto& prop : m_entries)
        {
            ...
            if (count < spacing)
            {
                stream << ustring_t(spacing - count, U(' '));
            }
