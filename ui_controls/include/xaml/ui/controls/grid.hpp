...
namespace xaml
{
    ...
    struct grid_index
    {
        ...
        std::int32_t column_span, row_span;
    };
    ...
    inline std::vector<grid_length> __stogls(std::basic_string_view<TChar> str)
    {
        ...
        do
        {
            std::int32_t index = str.find_first_of(__delimeter<TChar>, offset);
    ...
    public:
        static void set_column(control& c, std::int32_t col) { m_indecies[&c].column = col; }
        static std::int32_t get_column(control& c) { return m_indecies[&c].column; }
        static void set_row(control& c, std::int32_t row) { m_indecies[&c].row = row; }
        static std::int32_t get_row(control& c) { return m_indecies[&c].row; }
        static void set_column_span(control& c, std::int32_t cs) { m_indecies[&c].column_span = cs; }
        static std::int32_t get_column_span(control& c) { return m_indecies[&c].column_span; }
        static void set_row_span(control& c, std::int32_t rs) { m_indecies[&c].row_span = rs; }
        static std::int32_t get_row_span(control& c) { return m_indecies[&c].row_span; }
