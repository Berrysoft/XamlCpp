...
xaml_result xaml_text_box_internal::draw(xaml_rectangle const& region) noexcept
{
    if (!m_handle)
    {
        ...
        gtk_text_view_set_wrap_mode(GTK_TEXT_VIEW(m_handle), GTK_WRAP_WORD);
        GtkTextBuffer* buffer = gtk_text_view_get_buffer(GTK_TEXT_VIEW(m_handle));
...
void xaml_text_box_internal::on_changed(GtkTextBuffer*, xaml_text_box_internal* self) noexcept
{
    ...
    guard.test_and_set();
    GtkTextBuffer* buffer = gtk_text_view_get_buffer(GTK_TEXT_VIEW(self->m_handle));
