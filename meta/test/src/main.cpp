...
int main()
{
    ...
    XAML_THROW_IF_FAILED(ctx->get_type_by_name(xaml_box_value(U("xaml_test_calculator")).get(), (xaml_reflection_info**)&t));
    ...
    XAML_THROW_IF_FAILED(t->get_event(xaml_box_value(U("value_changed")).get(), &ev));
    ...
    XAML_THROW_IF_FAILED(ev->add(obj.get(), handler.get(), &token));
    {
        ...
        XAML_THROW_IF_FAILED(t->get_method(xaml_box_value(U("plus")).get(), &method));
    ...
    XAML_THROW_IF_FAILED(t->get_property(xaml_box_value(U("value")).get(), &prop));
    ...
    XAML_THROW_IF_FAILED(prop->set(obj.get(), xaml_box_value(U("200")).get()));
    // Remove the handler.
    XAML_THROW_IF_FAILED(ev->remove(obj.get(), token));
    {
        ...
        xaml_ptr<xaml_method_info> method;
        XAML_THROW_IF_FAILED(t->get_method(xaml_box_value(U("minus")).get(), &method));
        xaml_ptr<xaml_vector_view> args;
        XAML_THROW_IF_FAILED(xaml_delegate_pack_args(&args, obj, 1, 1));
        xaml_ptr<xaml_object> res;
        XAML_THROW_IF_FAILED(method->invoke(args.get(), &res));
